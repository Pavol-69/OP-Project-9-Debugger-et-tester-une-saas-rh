...
export default class Login {
  ...
  handleSubmitAdmin = (e) => {
    ...
    const user = {
      type: "Admin",
      email: e.target.querySelector(`input[data-testid="admin-email-input"]`) // BUG #2, il y avait employe ald admin
        .value,
      password: e.target.querySelector(
        `input[data-testid="admin-password-input"]` // BUG #2, il y avait employe ald admin
      ).value,
      status: "connected",
    };
    this.localStorage.setItem("user", JSON.stringify(user));
